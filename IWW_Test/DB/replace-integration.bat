...
php "..\..\tools\Search-Replace-DB-master\srdb.cli.php" -h alabang1.hankeyinvestments.dev0 -u DevMySQL_dev0 -p Testing.Dev0 -n temp_migrate_%num% -s "C:\\Azure\\%local_folder_name%" -r "C:\\inetpub\\WordPress\\%public_host%"
del %filename_prefix%-Integration.sql
..\..\tools\mysql-5.6.25\mysqldump.exe --skip-extended-insert --user=DevMySQL_dev0 --password=Testing.Dev0 --max_allowed_packet=1G --host=alabang1.hankeyinvestments.dev0 --port=3306 --default-character-set=utf8 "temp_migrate_%num%" --result-file=%filename_prefix%-Integration.sql
..\..\tools\mysql-5.6.25\mysql --user=DevMySQL_dev0 --password=Testing.Dev0 --host=alabang1.hankeyinvestments.dev0 --execute="drop database if exists temp_migrate_%num%"