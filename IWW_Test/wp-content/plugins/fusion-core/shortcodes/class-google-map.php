...
class FusionSC_GoogleMap {
    ...
    function render( $args, $content = '' ) {
		...
		if( $address ) {
			...
			ob_start(); ?>
			<input type='hidden' name='hf_site_map' id='hf_site_map' value='map_<?php echo $map_id; ?>'>
			<input type='hidden' name='hf_source_lat' id='hf_source_lat' value=''>
			<input type='hidden' name='hf_source_lng' id='hf_source_lng' value=''>
			<script type="text/javascript">
				var map_<?php echo $map_id; ?>;
				var latitude = "<?php echo $coordinates[0]['lat']; ?>";
				var longitude = "<?php echo $coordinates[0]['lng']; ?>";
				var markers = [];
				var counter = 0;
				...
				function fusion_run_map_<?php echo $map_id ; ?>() {
					...
					//load set current location, interacts w/ main js
					if(navigator.geolocation)
					{
						navigator.geolocation.getCurrentPosition(function(position) {
							...
							map_<?php echo $map_id ; ?>.setCenter(pos);
							
							//abort last call if exist, force new call to dealers
							if (window.map_xhr != undefined) {
								window.map_xhr.abort();
							}
							window.map_trigger_monitor==true;
							dealers.getNearestDealers("coords", "", "", "", "", position.coords.latitude, position.coords.longitude, "coords", true, true);	 
							setTimeout(function(){ window.map_trigger_monitor==false; }, 3000);
							
							//settimeout to bound map
							setTimeout(function(){ window.map_trigger_monitor = true; fitLocationToMap(position.coords.latitude, position.coords.longitude); window.map_trigger_monitor = false; }, 4000);
